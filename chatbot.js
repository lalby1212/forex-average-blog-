...
(function () {
  ...
  /* ── CONFIG ── */
  const CONFIG = {
    API_KEY: "AIzaSyA9k94C3CFIazWoviF1liOtVqXmtq7pYgw",
...
`;
  ...
  function injectCSS() {
    ...
    .fa-cta-btn--gold:hover { background: #FFE552 !important; box-shadow: 0 4px 18px rgba(255,215,0,0.45); }
    ...
    .fa-cta-btn--green:hover { background: #15803d !important; }
    ...
    .fa-cta-btn--outline:hover { background: rgba(255,215,0,0.1) !important; }
  ...
  async function callGemini(userMessage) {
    // Injecter le system prompt comme premier message utilisateur
    // car v1beta/gemini-1.5 supporte system_instruction mais pas tous les modèles
    // Solution universelle : on préfixe le system prompt dans la conversation
    const fullHistory = [
      {
        role: "user",
        parts: [{ text: "INSTRUCTIONS SYSTÈME (à suivre absolument) :\n" + SYSTEM_PROMPT + "\n\nRéponds: Compris, je suis prêt." }]
      },
      {
        role: "model",
        parts: [{ text: "Compris, je suis l'IA officielle de Forex Average. Je suis prêt à aider la communauté." }]
      },
      ...conversationHistory,
    ...
    for (const model of MODELS_TO_TRY) {
      try {
        ...
        console.info(`[FA Chatbot] ✅ Réponse via ${model}`);
  ...
  async function sendMessage(message) {
    ...
    try {
      ...
      hideTyping();
      console.error("[FA Chatbot] Erreur finale :", err.message);
      addMessage("bot",
        `Désolé, je rencontre un problème technique. 😕<br><br>
        Pour une aide immédiate, rejoins notre communauté :<br>
        <a href="${CONFIG.LINKS.telegram_free}" target="_blank" rel="noopener noreferrer" class="fa-cta-btn fa-cta-btn--green" style="color:#fff!important;">📱 Telegram Gratuit</a>
        <a href="${CONFIG.LINKS.telegram_vip}" target="_blank" rel="noopener noreferrer" class="fa-cta-btn fa-cta-btn--green" style="color:#fff!important;">💎 Telegram VIP</a>
        <span class="fa-legal">⚠️ Le trading comporte des risques de perte en capital.</span>`,
        true
      );
    }
