...
(function () {
  ...
  /* ── CONFIG ── */
  const CONFIG = {
    API_KEY: "VOTRE_CLE_ICI", // ← Remplace par ta nouvelle clé
  ...
  function init() {
    ...
    if (CONFIG.API_KEY === "VOTRE_CLE_ICI" || CONFIG.API_KEY.length < 20) {
      ...
      fetch(testUrl, {
        ...
        .then(d => {
          if (d?.error) {
            console.error("[FA Chatbot] ❌ Erreur API :", d.error.message);
            console.error("[FA Chatbot] Code :", d.error.code, "| Statut :", d.error.status);
          } else {
            console.info("[FA Chatbot] ✅ Clé API valide — gemini-1.5-flash opérationnel");
          }
        })
        .catch(e => console.error("[FA Chatbot] ❌ Erreur réseau :", e.message));
    }

    // Montre le dot de notification après 3s
    setTimeout(() => {
